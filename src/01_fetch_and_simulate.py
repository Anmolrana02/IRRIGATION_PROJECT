...
def fetch_weather_data_for_districts(districts, start, end):
    ...
    for district_name, (lat, lon) in districts.items():
        ...
        try:
            ...
            df = pd.DataFrame({
                "date": pd.to_datetime(data["T2M"].keys(), format='%Y%m%d'),
...
def calculate_eto_manual(df):
    ...
    try:
        ...
        phi = np.deg2rad(df['latitude']) # Use the specific latitude for each row
...
def run_multi_crop_simulation(df, crop_profiles):
    ...
    # This will hold the results from each district's simulation
    all_district_sims = []
    ...
    for district in df['district'].unique():
        ...
        simulated_moisture_list = []
        generated_irrigated_list = []
...
if __name__ == "__main__":
    ...
    if os.path.exists(FINAL_DATA_PATH):
        ...
        if df_weather is not None:
            ...
            # Save the final dataset for the next step
